...
export default function ArtistDashboard() {
  ...
  async function fetchArtistData() {
    ...
    setError(null);
    try {
      // Example endpoints — change as needed
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const [artistRes, tracksRes] = await Promise.all([
        axiosMusic.get("/artist/me", { headers }).catch(() => null),
        axiosMusic.get("/artist/tracks", { headers }).catch(() => null),
      ]);

      // adapt to response shape (data, artist)
      setArtist(artistRes?.data || artistRes?.data?.artist || null);
      setTracks(tracksRes?.data?.tracks || tracksRes?.data || tracksRes?.data?.items || []);
    } catch (e) {
  ...
  async function onUploadSubmit(formData) {
    ...
    try {
      ...
      body.append("album", formData.album || "");
      body.append("genre", formData.genre || "");
      if (formData.artwork && formData.artwork[0]) body.append("artwork", formData.artwork[0]);
      if (formData.audio && formData.audio[0]) body.append("audio", formData.audio[0]);
      body.append("explicit", formData.explicit ? "true" : "false");
      ...
      // endpoint: /artist/tracks (example)
      const res = await axiosMusic.post("/artist/tracks", body, { headers });

      if (res?.data) {
        // push new track into state (adapt shape)
        const newTrack = res.data.track || res.data;
  ...
  async function onDeleteTrack(trackId) {
    if (!confirm("Delete this track? This action cannot be undone.")) return;
    try {
      const headers = token ? { Authorization: `Bearer ${token}` } : {};
      const res = await axiosMusic.delete(`/artist/tracks/${trackId}`, { headers });
      if (res.status === 200 || res.status === 204) {
        setTracks(prev => prev.filter(t => (t.id || t._id) !== trackId));
      } else {
                  ...
                  <div className="p-6 md:p-8 flex items-center gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-4 flex gap-3">
                        <button onClick={() => setUploadOpen(true)} className="btn-primary py-2 px-4 rounded-lg flex items-center gap-2">
                          <Plus size={16} /> Upload Track
                        </button>
                        <button className="btn-ghost py-2 px-4 rounded-lg flex items-center gap-2">
                          Edit Profile
                        </button>
                      </div>
                    </div>
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-2xl glass p-6 rounded-xl">
            ...
            <form onSubmit={handleSubmit(onUploadSubmit)} className="mt-4 grid grid-cols-1 gap-4">
              ...
              <div className="flex items-center gap-3 justify-end mt-2">
                <button type="button" onClick={() => setUploadOpen(false)} className="btn-ghost px-4 py-2 rounded">Cancel</button>
                <button type="submit" className="btn-primary px-4 py-2 rounded flex items-center gap-2" disabled={uploading}>
                  {uploading ? "Uploading..." : <><Upload /><span>Upload</span></>}
                </button>
              </div>
            </form>
          </motion.div>
