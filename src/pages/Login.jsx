...
// small inline input component tailored to your CSS
...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    if (res?.ok) {
      // allow auth context to settle then navigate
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4">
      <motion.div className="w-full max-w-md glass p-8 rounded-xl border border-white/10 shadow-xl" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <motion.div variants={itemVariants}>
            <motion.button
              type="submit"
              className="w-full btn-primary text-black py-3 rounded-lg font-semibold flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={loading}
